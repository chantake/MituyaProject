...
public class Pager {
    ...
    public static void page(PageInfo pi) {
        if (pi.page < 1 || pi.page > pi.pageCount) {
            ...
            s.sendMessage(infoColor + "---------------------------------------------------");
            if (pi.pageCount > 1) {
                s.sendMessage("Use " + ChatColor.YELLOW + (s instanceof Player ? "/" : "") + "rcp [page#|next|prev|last]" + ChatColor.WHITE + " to see other pages.");
            }
        }
    }
    ...
    private static int getCharWidth(char ch) {
        if (ch == ' ') {
            return SPACE_WIDTH;
        } else if (TWO_PIXEL_CHARS.indexOf(ch) != -1) {
            return 2;
        } else if (THREE_PIXEL_CHARS.indexOf(ch) != -1) {
            return 3;
        } else if (FOUR_PIXEL_CHARS.indexOf(ch) != -1) {
            return 4;
        } else if (FIVE_PIXEL_CHARS.indexOf(ch) != -1) {
            return 5;
        } else {
            return CHAR_WIDTH;
        }
