...
public class ParsingUtils {
    ...
    /**
     * Returns true if the string can be parsed as a number. Currently the method tries to parse the String using Double.parseDouble() and returns true if it
     * succeeds.
     *
    ...
    /**
     * Returns true if the string can be parsed as an integer number. Currently the method tries to parse the String using Integer.parseInteger() and returns
     * true if it succeeds.
     *
     * @param value The String that is checked.
     * @return
    ...
    /**
     * Returns true if the character in the index parameter is nested inside balanced characters. for example, using the String "a(b,c),(d,e[f]),g'abcdefg'":
     * isNested(str, 0, "(['",")]'") will return false while the same call with an index of 3 will return true. The length of openBalance must be exactly the
     * same as the length of closeBalance. each character in openBalance is paired with the character of the same index in closeBalance. For example if
     * openBalance="([" closeBalance should be ")]" if the same type of brackets are to be paired.
     *
    ...
    public static boolean isNested(String str, int index, String openBalance, String closeBalance) throws IllegalArgumentException {
        ...
        if (closers.length != openers.length) {
            throw new IllegalArgumentException("There must be the same number of balance opener and balance closer characters.");
        }
    ...
    public static String parseEscapeCharacters(String str) {
        return str.replaceAll("\\$gt\\.", ">").replaceAll("\\$lt\\.", "<").replaceAll("\\$amp\\.", "&").replaceAll("\\$sc\\.", ";").replaceAll("\\$eq\\.", "=").replaceAll("\\$com\\.", ",").replaceAll("\\$apos\\.", "'").replaceAll("\\$curl-open\\.", "{").replaceAll("\\$curl-close\\.", "}").replaceAll("\\$under\\.", "_").replaceAll("\\$tab\\.", "\t").replaceAll("\\$nline\\.", System.getProperty("line.separator"));
    }
    ...
    private static final boolean backSpcSign = true;
