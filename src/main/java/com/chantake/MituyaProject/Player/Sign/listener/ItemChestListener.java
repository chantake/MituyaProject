...
public class ItemChestListener implements Listener {
    ...
    private final ItemChestManager manager;

    public ItemChestListener(ItemChestManager manager) {
    ...
    public void MituyaPlayerSignClickEvent(MituyaPlayerSignClickEvent event) {
        ...
        if (event.isRightClick()) {
            event.setCancel(manager.doEvent(event.getPlayer(), event.getPlayerInstance(), event.getSign()));
        }
    }
