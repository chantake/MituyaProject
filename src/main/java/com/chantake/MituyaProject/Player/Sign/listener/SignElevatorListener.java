...
public class SignElevatorListener implements Listener {
    ...
    private final SignElevatorManager manager;

    public SignElevatorListener(SignElevatorManager manager) {
    ...
    public void MituyaPlayerSignClickEvent(MituyaPlayerSignClickEvent event) {
        ...
        if (event.isRightClick()) {
            event.setCancel(this.manager.doElevator(event.getPlayer(), event.getPlayerInstance(), event.getSign()));
        }
    }
}
