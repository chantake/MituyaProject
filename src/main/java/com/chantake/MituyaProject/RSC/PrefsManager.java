...
public class PrefsManager {
    ...
    private static final String prefsFileName = "rscpreferences.yml";
    ...
    public enum Prefs {
        ...
        inputBlockType, outputBlockType, interfaceBlockType, infoColor, errorColor, debugColor,
    ...
    public PrefsManager(RedstoneChips plugin) {
        ...
        try {
            ...
            defaults = (Map<String, Object>)yaml.load(new FileInputStream(propFile));
        }
    ...
    public void loadPrefs() {
        ...
        try {
            ...
            if (loadedPrefs == null) {
                loadedPrefs = new HashMap<String, Object>();
            }
    ...
    public void savePrefs() {
        if (!rc.getDataFolder().exists()) {
            rc.getDataFolder().mkdir();
        }
    ...
    /**
     * Adds a chunk of yaml to the preferences Map. Parses the yaml into a Map object and sets the preference values according to the map. Only changing the
     * value of EXISTING preferences keys is allowed.
     *
    ...
    public void printYaml(CommandSender sender, Map<String, Object> map) {
        ...
        sender.sendMessage("");
        sender.sendMessage(getInfoColor() + rc.getPlugin().getDescription().getName() + " " + "BuildInRSC" + " preferences:");
    ...
    /**
     * Tries to find a material according to search string m. The string can be either a material name or a name and data value combination such as: wool:orange
     * or wood:1.
     *
     * @param m
    ...
    /**
     * Allows circuit libraries to register their own preferences keys. This method should be called in the CircuitIndex onRedstoneChipsEnable() method to
     * ensure the key is added before RedstoneChips reads the preferences file. The preferences key is set in the format of class-name.key
     *
