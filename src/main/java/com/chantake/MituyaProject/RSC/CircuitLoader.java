...
public class CircuitLoader {
     ...
     */
    private final Map<String, Class<? extends Circuit>> circuitClasses = new HashMap<>();

    private final List<CircuitIndex> libraries = new ArrayList<>();
    ...
    public Circuit getCircuitInstance(String name) throws InstantiationException, IllegalAccessException {
        ...
        if (c == null) {
            throw new IllegalArgumentException("Unknown circuit type: " + name);
        } else {
            return (Circuit)c.newInstance();
        }
    }
