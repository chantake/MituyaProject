 ...
 *
 * This class implements a vector of bits that grows as needed. Each component of the bit set has a {@code boolean} value. The bits of a {@code BitSet} are
 * indexed by nonnegative integers. Individual indexed bits can be examined, set, or cleared. One {@code BitSet} may be used to modify the contents of another
 * {@code BitSet} through logical AND, logical inclusive OR, and logical exclusive OR operations.
 *
 * <p>
 * By default, all bits in the set initially have the value {@code false}.
 *
 * <p>
 * Every bit set has a current size, which is the number of bits of space currently in use by the bit set. Note that the size is related to the implementation
 * of a bit set, so it may change with implementation. The length of a bit set relates to logical length of a bit set and is defined independently of
 * implementation.
 *
 * <p>
...
public class BitSet7 implements Cloneable, java.io.Serializable {
     ...
     *
     * The bits in this BitSet. The ith bit is stored in bits[i/64] at bit position i % 64 (where bit position 0 refers to the least significant bit and 63
    ...
    /**
     * Sets the field wordsInUse to the logical size in words of the bit set. WARNING:This method assumes that the number of words actually in use is less than
     * or equal to the current value of wordsInUse!
     */
    ...
    /**
     * Creates a bit set whose initial size is large enough to explicitly represent bits with indices in the range {@code 0} through {@code nbits-1}. All bits
     ...
     *
     * @param lb a long buffer containing a little-endian representation of a sequence of bits between its position and limit, to be used as the initial bits of
     * the new bit set
     * @return
     * @since 1.7
     */
     ...
     *
     * @param bb a byte buffer containing a little-endian representation of a sequence of bits between its position and limit, to be used as the initial bits of
     * the new bit set
     * @return
     * @since 1.7
     */
    ...
    /**
     * Ensures that the BitSet can accommodate a given wordIndex, temporarily violating the invariants. The caller must restore the invariants before returning
     * to the user, possibly using recalculateWordsInUse().
     *
    ...
    public BitSet7 get(int fromIndex, int toIndex) {
        ...
        result.words[targetWords - 1]
                ...
                ? /* straddles source words */ ((words[sourceIndex] >>> fromIndex)
     ...
     * only if it either already had the value {@code true} or the corresponding bit in the bit set argument has the value {@code true}.
     *
     * @param set a bit set
     */
    ...
    /**
     * Performs a logical <b>XOR</b> of this bit set with the bit set argument. This bit set is modified so that a bit in it has the value {@code true} if and
     ...
     *         h ^= words[i] * (i + 1);
     *     return (int)((h >> 32) ^ h);
     * }}</pre> Note that the hash code changes if the set of bits is altered.
     *
    ...
    /**
     * Returns the number of bits of space actually in use by this {@code BitSet} to represent bit values. The maximum element in the set is the size - 1st
     * element.
     *
    ...
    /**
     * Compares this object against the specified object. The result is {@code true} if and only if the argument is not {@code null} and is a {@code Bitset}
     * object that has exactly the same set of bits set to {@code true} as this bit set. That is, for every nonnegative {@code int} index {@code k},
     * <pre>((BitSet)obj).get(k) == this.get(k)</pre> must be true. The current sizes of the two bit sets are not compared.
     *
    ...
    /**
     * Cloning this {@code BitSet} produces a new {@code BitSet} that is equal to it. The clone of the bit set is another bit set that has exactly the same bits
     * set to {@code true} as this bit set.
     *
    ...
    /**
     * Attempts to reduce internal storage used for the bits in this bit set. Calling this method may, but is not required to, affect the value returned by a
     * subsequent call to the {@link #size()} method.
     */
     ...
     * representation of that index is included in the result. Such indices are listed in order from lowest to highest, separated by ",&nbsp;" (a comma and a
     * space) and surrounded by braces, resulting in the usual mathematical notation for a set of integers.
     *
     * <p>
     * Example:
     * <pre>
     * BitSet drPepper = new BitSet();</pre> Now {@code drPepper.toString()} returns "{@code {}}"
     * .<p>
     * <
     * pre>
     ...
     * pre>
     * drPepper.set(4);
     * drPepper.set(10);</pre> Now {@code drPepper.toString()} returns "{@code {2, 4, 10}}".
     *
