...
public class CommandUtils {
    ...
    /**
     * Checks whether the player is pointing towards a chip block. Sends an error message to the player in case no chip was found.
     *
     * @param rc
     * @param sender
     * @param distance
    ...
    public static boolean checkPermission(RedstoneChips rc, CommandSender sender, String commandName, boolean opRequired, boolean report) {
        if (!rc.getPrefs().getUsePermissions()) {
            return (opRequired ? sender.isOp() : true);
        }
        if (!(sender instanceof Player)) {
            return true;
        }
        ...
        if (rc.getPlugin().permission.has(player, "redstonechips.command." + commandName)) {
            return true;
        } else {
            if (report) {
                sender.sendMessage(rc.getPrefs().getErrorColor() + "You do not have permission to use command " + commandName + ".");
            }
            return false;
        }
    }
