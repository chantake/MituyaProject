...
public class ChipManager {
    ...
    public ChipManager(RedstoneChips plugin) {
        rc = plugin;
    }
    ...
    public boolean destroyChip(Chip destroyed, CommandSender destroyer, boolean destroyBlocks, boolean updateSign) {
        if (destroyBlocks) {
            ...
            if (!enableDestroyCommand) {
                if (destroyer != null)
                    destroyer.sendMessage(RCPrefs.getErrorColor() + "/rcdestroy is disabled. You can enable it using /rcprefs enableDestroyCommand true");
                return false;
            }
        }
        ...
        if (!RCPermissions.checkChipPermission(destroyer, destroyed.getType(), false)) {
            if (destroyer != null)
                destroyer.sendMessage(RCPrefs.getErrorColor() + "You do not have permission to destroy circuits of type " + destroyed.getType() + ".");
            return false;
        }
    ...
    public boolean resetChip(Chip c, CommandSender reseter) {
        ...
        if (mChip == MaybeChip.AChip) {
            ...
            if (activateChip(newChip, reseter, id)) {
                ...
                if (reseter != null)
                    reseter.sendMessage(RCPrefs.getInfoColor() + "Successfully reactivated " + ChatColor.YELLOW + newChip + RCPrefs.getInfoColor() + ".");
                return true;
            } else return false;
        } else {
            return false;
