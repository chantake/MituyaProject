 ...
 * a Map for serialization, as well as, turning a Map object back into a {@link com.chantake.MituyaProject.RSC.Chip.Chip Chip}.
 *
 * @author taleisenberg
 */
public class ChipSerializer extends Serializer {
    ...
    public MaybeChip deserialize(Map<String, Object> map) {
        ...
        if (mChip != MaybeChip.AChip) return mChip; // fail
            ...
            if (map.containsKey(Key.OUTPUT_BITS.key))
                updateOutputPins(c, BooleanArrays.fromString((String) map.get(Key.OUTPUT_BITS.key)));
