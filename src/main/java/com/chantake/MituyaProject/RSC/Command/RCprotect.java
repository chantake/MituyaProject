...
public class RCprotect extends RCCommand {
    ...
    private void channelInfo(CommandSender sender, String channelName) {
        ...
        if (curChannel.isProtected()) {
            ...
            if (!owners.isEmpty())
                if (sender != null)
                    sender.sendMessage(RCPrefs.getInfoColor() + "admins: " + extraColor + owners.substring(0, owners.length() - 2));
            if (!users.isEmpty())
                if (sender != null)
                    sender.sendMessage(RCPrefs.getInfoColor() + "users: " + extraColor + users.substring(0, users.length() - 2));
        } else {
            if (sender != null)
                sender.sendMessage(RCPrefs.getErrorColor() + "Channel " + channelName + " is not protected.");
        }
    }
    ...
    private void protect(CommandSender sender, String[] args) {
        ...
        if (!curChannel.isProtected()) {
            if (!(sender instanceof Player) && args.length < 3) {
                if (sender != null)
                    sender.sendMessage(RCPrefs.getErrorColor() + "Usernames must be specified if run from console.");
                return;
            }
            ...
            if (sender != null)
                sender.sendMessage(RCPrefs.getInfoColor() + "Channel " + args[0] + " has been protected.");
        } else {
            if (sender != null)
                sender.sendMessage(RCPrefs.getErrorColor() + "Channel " + args[0] + " is already protected.");
        }
    }
    ...
    private void unprotect(CommandSender sender, String[] args) {
        if (rc.channelManager().getBroadcastChannels().containsKey(args[0])) {
            ...
            if (curChannel.isProtected()) {
                ...
                if (sender != null)
                    sender.sendMessage(RCPrefs.getInfoColor() + "Channel " + args[0] + " has been unprotected.");
            } else {
                if (sender != null)
                    sender.sendMessage(RCPrefs.getErrorColor() + "Channel " + args[0] + " is not protected.");
            }
        } else {
        ...
        if (rc.channelManager().getBroadcastChannels().containsKey(args[0])) {
            ...
            if (curChannel.isProtected()) {
                if (args.length > 2) {
                    ...
                    if (sender != null)
                        sender.sendMessage(RCPrefs.getInfoColor() + "Channel " + args[0] + " has been updated.");
                } else {
    ...
    private void listChannels(CommandSender sender) {
        ...
        if (!protectedChannels.isEmpty()) {
            if (sender != null)
                sender.sendMessage(RCPrefs.getInfoColor() + "Protected Channels: " + protectedChannels.substring(0, protectedChannels.length() - 2));
        } else {
