...
import com.chantake.MituyaProject.RSC.Circuit.CircuitListener;
...
import com.chantake.MituyaProject.RSC.Circuit.Scan.ChipScanner.ChipScanException;
import com.chantake.MituyaProject.RSC.Circuit.Scan.IOChipScanner;
import com.chantake.MituyaProject.RSC.Circuit.Scan.RecursiveChipScanner;
import com.chantake.MituyaProject.RSC.Circuit.Scan.ScanParameters;
import com.chantake.MituyaProject.RSC.Circuit.Scan.SingleBlockChipScanner;
...
public class CircuitManager implements Listener {
    ...
    private final Map<ChunkLocation, List<Circuit>> chunkLookupMap = new HashMap<>();
    ...
    private final Map<Location, Circuit> structureLookupMap = new HashMap<>();
    private final Map<Location, Circuit> activationLookupMap = new HashMap<>();
    private final List<ChunkLocation> processedChunks = new ArrayList<>();
    ...
    /**
     * Redstone change event handler. Checks if this event reports an input change in any circuit's input pins. When the new redstone state is different than
     * the current one, the input pin is updated and the circuit is notified.
     *
     * @param b
     * @param newCurrent
     * @param oldCurrent
    ...
    public int checkForCircuit(ScanParameters params, CommandSender sender, int debugLevel) {
        if (params == null) {
            if (sender != null) {
                sender.sendMessage(rc.getPrefs().getErrorColor() + "Weird error occured: params==null.");
            }
            return -2;
        }
        ...
        if (!rc.getCircuitLoader().getCircuitClasses().containsKey(signClass)) {
            return -1;
        }
        ...
        if (check != null) {
            if (sender != null) {
                sender.sendMessage(rc.getPrefs().getInfoColor() + "Chip is already active - " + check.getChipString() + ".");
            }
            return -2;
        }
        ...
        if (params == null || (params.outputs.isEmpty() && params.inputs.isEmpty() && params.interfaces.isEmpty())) {
            return -1;
        }
    ...
    public boolean checkCircuitDestroyed(Block b, CommandSender s) {
        ...
        if (destroyed != null && circuits.containsValue(destroyed)) {
            if (destroyCircuit(destroyed, s, false)) {
                if (s != null) {
                    s.sendMessage(rc.getPrefs().getErrorColor() + "You destroyed " + destroyed.getChipString() + ".");
                }
            } else {
                return false;
            }
        ...
        if (dataClass != null && redstoneClass.isAssignableFrom(dataClass)) {
            ...
            if (inputs != null) {
                for (InputPin pin : inputs) {
                    if (isBroken) {
                        pin.updateValue(block, false, SourceType.REDSTONE);
                    } else {
                        pin.updateValue(block, pin.findSourceBlockState(block.getLocation()), SourceType.REDSTONE);
                    }
                }
                return true;
            }
    ...
    boolean checkCircuitOutputBlockPlaced(Block block, Player player) {
        if (OutputPin.isOutputMaterial(block.getType())) {
            ...
            if (outputs != null && !outputs.isEmpty()) {
                /*
                 * rc.getServer().getScheduler().scheduleSyncDelayedTask(rc, new
    ...
    /**
     * Resets a circuit. First the circuit is destroyed and then reactivated. Any listeners of the circuit are copied over to the new circuit.
     *
    ...
    public boolean resetCircuit(Circuit c, CommandSender reseter) {
        ...
        if (!rc.getCircuitManager().destroyCircuit(c, reseter, false)) {
            return false;
        }
        ...
        if (newCircuit != null) {
            ...
            if (reseter != null) {
                reseter.sendMessage(rc.getPrefs().getInfoColor() + "Successfully reactivated " + ChatColor.YELLOW + newCircuit.getChipString() + rc.getPrefs().getInfoColor() + ".");
            }
            return true;
        } else {
            return false;
    ...
    /**
     * Check each active circuit to see if all of its blocks are in place. See Circuit.checkIntegrity(). Any unloaded circuit chunks are first loaded and then
     * unloaded after the check is over.
     *
     * @param world
     */
    ...
    public HashMap<Integer, Circuit> getCircuits(World world) {
        HashMap<Integer, Circuit> worldCircuits = new HashMap<>();
    ...
    public Circuit getCircuitById(String id) {
        ...
        if (ParsingUtils.isInt(id)) { // as id number
            ...
            for (Circuit c : circuits.values()) {
                if (c.name != null && c.name.equals(id)) {
                    return c;
                }
            }
        }
    ...
    private boolean checkChipPermission(CommandSender sender, String classname, boolean create) {
        ...
        return this.rc.getPlugin().permission.has(player, "redstonechips.circuit." + (create ? "create." : "destroy.") + classname);
    }

    /**
     * Mark a chunk as a processed chunk. While marked the chunk would not be allowed to unload. Call releaseChunk(chunk) to release it.
     *
