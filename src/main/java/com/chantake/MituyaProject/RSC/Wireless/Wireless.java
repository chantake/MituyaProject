...
public abstract class Wireless {
    ...
    /**
     * Sets the Wireless channel length in bits. This is the maximum number of bits the Wireless will use and doesn't necessarily needs to be the same as the
    ...
    public void init(CommandSender sender, String channelString, int channelLength, Circuit c) throws IllegalArgumentException {
        ...
        if (sender != null) {
            ...
            if (this.getChannelLength() > 1) {
                ...
                bits = "bit " + ChatColor.YELLOW + this.getStartBit();
            }
    ...
    private boolean checkChanUserPermissions(CommandSender sender, String name) {
        ...
        if (!(rc.getChannelManager().getChannelByName(name, false).isProtected())) {
            return true;
        }
    ...
    public void shutdown() {
        if (getChannel() != null) {
            if (this instanceof Receiver) {
                rc.getChannelManager().removeReceiver((Receiver)this);
            } else if (this instanceof Transmitter) {
                rc.getChannelManager().removeTransmitter((Transmitter)this);
            }
        }
    }
}
