...
import com.chantake.MituyaProject.RSC.Circuits.SignWriter.DisplayMode;
import com.chantake.MituyaProject.RSC.Circuits.SignWriter.Type;
...
public class print extends Circuit implements RCTypeReceiver {
    ...
    public boolean init(CommandSender sender, String[] args) {
        ...
        if (channel == null && !checkInputs(sender, display)) {
            return false;
        }
        ...
        if (display == DisplayMode.replace) {
            dataPin = 1;
        } else if (display == DisplayMode.add) {
            dataPin = 2;
        ...
        return channel == null || initReceiver(sender, channel, type);
    }
