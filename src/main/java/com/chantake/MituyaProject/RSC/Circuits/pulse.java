...
public class pulse extends Circuit {
    ...
    private void pulse(final int idx) {
        ...
        if (interval == 0) {
            ...
            redstoneChips.getServer().getScheduler().scheduleSyncDelayedTask(redstoneChips.getPlugin(), pulseOffs[idx], intervalInTicks);
        }
    ...
    private void pulseSequence() {
        if (interval == 0) {
            ...
            sendOutput(0, true);
            redstoneChips.getServer().getScheduler().scheduleSyncDelayedTask(redstoneChips.getPlugin(), pulseOffs[0], intervalInTicks);
        }
    }
    ...
    class PulseOff implements Runnable {
        ...
        public void run() {
            ...
            if (inputs.length != outputs.length && pulseOffs.length > index + 1) {
                ...
                redstoneChips.getServer().getScheduler().scheduleSyncDelayedTask(redstoneChips.getPlugin(), pulseOffs[index + 1], intervalInTicks);
            }
        }
    }
}
