...
import com.chantake.MituyaProject.RSC.Memory.Ram;
import com.chantake.MituyaProject.RSC.Memory.RamListener;
import com.chantake.MituyaProject.Tool.Page.LineSource;
import com.chantake.MituyaProject.Tool.Page.Pager;
import com.chantake.MituyaProject.Tool.Parsing.Range;
...
public class sram extends Circuit implements RCTypeReceiver, RamListener {
    ...
    public void inputChange(int inIdx, boolean state) {
        if (inIdx == readWritePin) {
            ...
            if (readWrite && !sramDisable) { // store current data inputs when readWrite goes high.
                ...
                if (hasListeners()) {
                    debug("Writing " + BitSetUtils.bitSetToBinaryString(data, 0, wordLength) + " to address " + BitSetUtils.bitSetToUnsignedInt(address, 0, addressLength));
                }
            ...
            sramDisable = state;
            if (hasListeners()) {
                debug("Chip " + (sramDisable ? "disabled." : "enabled"));
            }
            if (sramDisable) {
                outputBits.clear();
            } else {
    ...
    private void readMemory() {
        ...
        if (hasListeners()) {
            debug("Reading " + BitSetUtils.bitSetToBinaryString(data, 0, wordLength) + " from address " + BitSetUtils.bitSetToUnsignedInt(address, 0, addressLength));
        }
    ...
    private BitSet7 parseData(CommandSender sender, String data) {
        try {
            ...
            if (data.length() == 1) {
                return BitSetUtils.intToBitSet((int)data.charAt(0), wordLength);
    ...
    private void dumpMemory(Player player, String srange) {
        ...
        if (firstAddress >= 0 && lastAddress >= 0) {
            ...
            if (firstAddress == lastAddress) {
                ...
                titleRange = firstAddress + "-" + lastAddress;
            }
    ...
    public void dataChanged(Ram ram, BitSet7 address, BitSet7 data) {
        ...
        if (readWrite && curaddr.equals(address)) {
            readMemory();
        }
    }
    ...
    class MemoryLineSource implements LineSource {
        ...
        public String getLine(int idx) {
            ...
            if (wordLength > 32) {
                value = Integer.toHexString(BitSetUtils.bitSetToSignedInt(data, 0, wordLength));
            } else {
                value = BitSetUtils.bitSetToBinaryString(data, 0, wordLength);
            }
        ...
        private String zeroPad(int a, int max) {
            ...
            for (int i = 0; i < charCount - address.length(); i++) {
                pad += "0";
            }
            return pad + address;
        }
