...
public class synth extends Circuit {
    ...
    protected boolean init(CommandSender sender, String[] args) {
        ...
        if (channel != null) {
            int len;
            if (indexedPitch) {
                len = (int)Math.ceil(Math.log(pitchIndex.length) / Math.log(2));
            } else {
                len = 5;
            }
    ...
    private void playNote(BitSet7 bits, int start, int length) {
        ...
        if (indexedPitch) {
            ...
            if (index >= pitchIndex.length) {
                if (hasDebuggers()) {
                    debug("pitch index out of bounds: " + index);
                }
                return;
            }
            ...
            if (val >= 24) {
                if (hasDebuggers()) {
                    debug("pitch value is too high: " + val);
                }
            }
            pitch = (byte)val;
        }
        ...
        if (pitch == -1) { // rest
            if (hasDebuggers()) {
                debug("Setting note blocks to rest");
            }
        } else {
            if (hasDebuggers()) {
                debug("Setting note blocks pitch to " + dataToNoteString(pitch) + " (" + pitch + ")");
            }
        ...
        if (note.matches("[\\-0-9]+")) { // the whole string is a number
            ...
            if (key.substring(0, 1).matches("[a-gA-G]")) {
                if (key.charAt(0) == 'c') {
                    ...
                    throw new IllegalArgumentException("Bad note name " + note);
                }
                if (key.length() > 1) {
                    if (key.charAt(1) == '#') {
                        keynum++;
                ...
                if (keynum > 24 || keynum < 0) {
                    throw new IllegalArgumentException(note + " is out of bounds. (" + keynum + "). The pitch range should be f#1 to f#3 or 0 to 24.");
                }
            } else {
                throw new IllegalArgumentException("Bad note name: " + note);
            }
