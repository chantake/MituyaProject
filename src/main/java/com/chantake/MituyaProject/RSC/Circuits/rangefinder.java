...
public class rangefinder extends Circuit {
    ...
    private Location findFaceCenter(Location l, BlockFace face) {
        if (face == BlockFace.DOWN) {
            ...
            throw new IllegalArgumentException("Invalid direction: " + face.name());
        }
    }
        ...
        for (BlockFace face : faces) {
            ...
            if (b.getType() == interfaceBlockType.getItemType()
                ...
                if (ret == null) {
                    ...
                    throw new IllegalArgumentException("Found too many interface blocks attached to each other.");
                }
            }
    ...
    private void trigger() {
        List<Location> objectsInRange = new ArrayList<>();
        ...
        if (dist <= range && dist > 0) {
            if (hasListeners()) {
                debug("Found object at " + debugFormat.format(dist) + " meters.");
            }
        } else {
            debug("No object found in range.");
        }
