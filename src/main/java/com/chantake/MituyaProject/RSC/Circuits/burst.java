...
public class burst extends Circuit {
    ...
    private void startBurst() {
        ...
        if (hasDebuggers()) {
            debug("Starting Burst.");
        }

        bursting = true;
        currentcount = 0;
        taskId = redstoneChips.getServer().getScheduler().scheduleSyncDelayedTask(redstoneChips.getPlugin(), burstTask);
        if (taskId == -1) {
            if (hasDebuggers()) {
    ...
    private class BurstTask implements Runnable {
        ...
        public void run() {
            ...
            int id = redstoneChips.getServer().getScheduler().scheduleSyncDelayedTask(redstoneChips.getPlugin(), burstTask, tickDelay);
