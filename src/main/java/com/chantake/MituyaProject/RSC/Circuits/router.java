            ...
            for (int i = 0; i < inputs.length - firstDataPin; i++) {
                ...
                if (outs != null) { // when there's no table entry do nothing.
                    for (int k : outs) {
                        if (k == -1) {
                            if (hasDebuggers()) {
                                debug("Routing data input " + i + " (" + (inputBits.get(i + firstDataPin) ? "on" : "off") + ") to all outputs.");
                            }
                            ...
                            if (hasDebuggers()) {
                                debug("Routing data input " + i + " (" + (inputBits.get(i + firstDataPin) ? "on" : "off") + ") to output " + k + ".");
                            }
    ...
    protected boolean init(CommandSender sender, String[] args) {
        ...
        for (String arg : args) {
            ...
            try {
                ...
                if (input >= inputs.length - firstDataPin) {
                    error(sender, "Data input " + input + " doesn't exist");
                }
