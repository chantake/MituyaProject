...
public class SignWriter {
    ...
    public void write(String text) {
        if (display == DisplayMode.add) {
            add(text);
            prepWrapLines();
            if (textBuffer.length() > LineWidth * 4) {
                textBuffer.setLength(0);
            }
    ...
    private void prepWrapLines() {
        if (textBuffer.length() > LineWidth * 3) {
            ...
            if (line4.length() > LineWidth) {
                line4 = line4.substring(0, LineWidth);
            }
    ...
    public static SignWriter getSignWriter(DisplayMode mode, Type type, Location... aroundBlocks) {
        ...
        for (Location loc : aroundBlocks) {
            ...
            if (checkBlock(i, north)) {
                signs.add(north);
            }
            if (checkBlock(i, south)) {
                signs.add(south);
            }
            if (checkBlock(i, west)) {
                signs.add(west);
            }
            if (checkBlock(i, east)) {
                signs.add(east);
            }
            if (checkBlock(i, up)) {
                signs.add(up);
