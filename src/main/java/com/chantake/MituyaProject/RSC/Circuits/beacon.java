...
public class beacon extends Circuit {
    ...
    protected boolean init(CommandSender sender, String[] args) {
        ...
        BuildInCircuitsListener.registerChunkbeaconCircuit(this);
    ...
    public void circuitShutdown() {
        BuildInCircuitsListener.deregisterChunkbeaconCircuit(this);
    }
    ...
    void onChunkUnload(ChunkUnloadEvent event) {
        if (isChunkInRange(ChunkLocation.fromChunk(event.getChunk()))) {
            ...
            if (keepalive) {
                if (hasDebuggers()) {
                    debug("Chunk (" + chunk.getX() + ", " + chunk.getZ() + ") in " + world.getName() + " is kept alive.");
                }
    ...
    private boolean isChunkInRange(ChunkLocation chunk) {
        if (chunk.getWorld().getUID() != world.getUID()) {
            return false;
        }
