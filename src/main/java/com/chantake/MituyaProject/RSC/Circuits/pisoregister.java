...
public class pisoregister extends Circuit {
    ...
    public void inputChange(int inIdx, boolean high) {
        if (inIdx == writePin) {
            ...
            if (shift) {
                ...
                debug("Switched to shift mode.");
            }
        } else if (inIdx == clockPin && !shift && high) {
            if (hasDebuggers()) {
                debug("Reading bit " + curIdx + ": " + register.get(curIdx));
            }
            sendOutput(0, register.get(curIdx));
            curIdx++;
            if (curIdx >= inputs.length - 2) {
                curIdx = 0;
            }
    ...
    private void write() {
        ...
        if (hasDebuggers()) {
            debug("writing " + BitSetUtils.bitSetToBinaryString(register, 0, inputs.length - 2) + " to register");
        }
    }
