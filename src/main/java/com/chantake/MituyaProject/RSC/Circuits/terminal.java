        ...
        if (args.length > 0) {
            ...
            if (args.length > 1) {
                if (args[1].equalsIgnoreCase("eot")) {
                    ...
                    error(sender, "Unknown argument: " + args[1]);
                }
            }
        }
    ...
    public void type(String[] words, Player player) {
        ...
        if (type == DataType.ascii) {
            ...
            for (int i = 0; i < typeString.length() - 1; i++) {
                ...
                if (hasDebuggers()) {
                    debug("Sending " + BitSetUtils.bitSetToBinaryString(outBuf, 0, 8) + "(" + (char)buf[0] + ")");
                }

                if (transmitter != null) {
                    transmitter.send(outBuf, 0, 8);
                } else {
            ...
            try {
                ...
                outBuf = BitSet7.valueOf(buf);
                if (hasDebuggers()) {
                    debug("Sending " + BitSetUtils.bitSetToBinaryString(outBuf, 0, outputs.length - 1));
                }
